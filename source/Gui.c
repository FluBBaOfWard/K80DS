...
#define EMUVERSION "V0.3.5 2022-08-23"
...
const fptr fnList8[] = {quickSelectGame, quickSelectGame, quickSelectGame, quickSelectGame, quickSelectGame};
...
void uiAbout() {
	...
	drawText(" Up:     Climb", 7, 0);
	drawText(" Y:      Squat", 8, 0);
	drawText(" B:      Attack", 9, 0);
	drawText(" A:      Power", 10, 0);

	drawText(" IronHorseDS " EMUVERSION, 20, 0);
...
void uiLoadGame() {
	...
	drawMenuItem(" Iron Horse (version K)");
	drawMenuItem(" Iron Horse (version H)");
	drawMenuItem(" Dai Ressya Goutou (Japan, version K)");
	drawMenuItem(" Far West");
	drawMenuItem(" Scooter Shooter");
}
